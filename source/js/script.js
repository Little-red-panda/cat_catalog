    ...
    if (sortPrice.classList.contains("sort__button--price-asc")) {
        for (let i = 0; i < cats.length; i++) {
            for (let j = i; j < cats.length; j++) {
                if (cats[i].getAttribute('data-price') > cats[j].getAttribute('data-price')) {
                    let swap = cats[i];
                    cats[i] = cats[j];
                    cats[j] = swap;
                }
...
sortAge.addEventListener('click', function () {
    sortAge.classList.toggle("sort__button--age-desc");

    if (sortAge.classList.contains("sort__button--age-desc")) {
        for (let i = 0; i < cats.length; i++) {
            for (let j = i; j < cats.length; j++) {
                if (cats[i].getAttribute('data-age') < cats[j].getAttribute('data-age')) {
                    let swap = cats[i];
                    cats[i] = cats[j];
                    cats[j] = swap;
                }
